...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('officer.update', [$users->id]) }}" style="font-size: 10px;">
                @csrf
                @method('PUT')
                ...
                <div class="form-group row mb-5">
                    ...
                    <div class="col-md-6">
                        <select class="custom-select" id="agency" name="id_agency">
                            <option value="" selected>__instansi__</option>
                            @foreach($agency as $x)
                                @if($x->id == $users->id_agency)
                                    @php($select = 'selected')
                                @else
                                    @php($select = '')
                                @endif
                            <option {{ $select }} value="{{ $x->id }}">{{ $x->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
